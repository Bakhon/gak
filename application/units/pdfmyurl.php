...
class PDFmyURL {
	...
	// Sets the margins to the values passed, but only if the margins are real numbers
	//
	// Returns true on success and false if the margins are not supported
	public function SetMargins($top, $bottom, $left, $right) {
		if (is_numeric($top) && is_numeric($bottom) && is_numeric($left) && is_numeric($right)) {
